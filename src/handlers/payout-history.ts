...
registerMainMenuItem({ label: "View payouts", data: "payouts:mine", order: 45 });
const composer = new Composer<Ctx>();
composer.callbackQuery("payouts:mine", async (ctx) => {
  await ctx.answerCallbackQuery();
  try {
    const payouts = await payoutsForUser(ctx.from.id);
    const text = payouts.length ? payouts.slice(0, 5).map((p) => `${p.amount} ${p.currency} — ${p.status}${p.transaction ? " — transaction recorded" : ""}`).join("\n") : "No payouts yet — link a verified wallet to receive automatic payouts.";
